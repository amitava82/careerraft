...
var ipLookup = require('../helpers/ip-lookup');
...
module.exports = function(deps){
    ...
    var geoOptions = {
        apiKey: deps.config.get('google.server'),
        formatter: null,
        region: 'in',
        addressdetails: 1
    };

    var geocoder = nodeGeocoder(geocoderProvider, httpsAdapter, geoOptions);
    ...
    return {
        ...
        Geo lookup
         */
        geoSearch: function (req, res, next) {
            geocoder.geocode(req.query.q).then(
                r => res.send(r),
                e => next(e, req, res)
            )
        },

        geoReverse: function (req, res, next) {
            geocoder.reverse({lat: req.query.lat, lon: req.query.lng}).then(
                r => res.send(r),
                e => next(e, req, res)
            )
        },
        
        ipLookup: function (req, res, next) {
            ipLookup(req.query.ip, function (err, resp) {
                if(err){
                    next(err, req, res);
                }else{
                    res.send(resp);
