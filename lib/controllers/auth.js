    ...
    return {
                ...
                .then(
                    id => {
                        var grid = new sendgrid.Email({
                            to: req.body.email,
                            from: 'donotreplay@educationalley.in',
                            fromname: 'Education Alley',
                            subject: 'Confirm your Education Alley account',
                            html: ' ',
                            text: ' '
                        });
            ...
            deps.redis.get('reset:'+code, function (err, email) {
                ...
                deps.models.User.findOne({email: email}).then(
                    user => {
                        ...
                        res.render('reset-password', {title: 'Reset password - Education Alley', data: { code: code}})
                    }
                ).onReject(
                    e => renderError(res, e)
                )
        ...
        updatePassword: function(req, res){
            ...
            if(!req.body.password || req.body.password !== req.body.confirmPassword)
                return res.render('reset-password', {title: 'Reset password - Education Alley', data: {code: code, error: 'Password do not match'}});
            ...
            deps.redis.get('reset:'+code, function (err, email) {
                ...
                deps.models.User.updatePassword(email, req.body.password).then(
                    () => {
                        ...
                        res.render('reset-password', {data: {success: true}, title: 'Reset password - Education Alley'});
                    },
                    e => renderError(res, e)
                )
            });
...
function renderError(res, e){
    res.render('error', {error: e.message, title: 'Error - Education Alley'})
}