...
module.exports = function(deps){
    ...
    orgSchema.statics.createBranch = function(parent, data){
        ...
        function createFromParent(source){
            var newOrg = _.extend({}, _.pick(source,
                ['name', 'description', 'email', 'short_description', 'logo', 'banner', 'business_hour', 'subjects']), data);
            return new self(newOrg).save();
        }

        function updateBranch(newOrg){
            return self.findByIdAndUpdate(parent, {
                $addToSet: {
                    branches: newOrg._id
                }
            }, {new: true}).exec();
